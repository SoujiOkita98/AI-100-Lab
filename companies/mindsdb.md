---
name: MindsDB
founded: 2017
headquarters: San Francisco, CA, USA
website: https://mindsdb.com
sector: AI / Database & ML Platform
one_liner: Open-source AI platform that brings machine learning capabilities directly into databases, enabling developers
  to build AI applications with SQL.
status: active
total_raised_m: 54
founders:
- name: Jorge Torres
  role: Co-Founder & CEO
- name: Adam Carrigan
  role: Co-Founder & COO
funding_rounds:
- stage: Seed
  date: 2017-12
  amount_m: 0.3
  lead_investors:
  - null
  source: https://www.crunchbase.com/organization/mindsdb
- stage: Seed
  date: 2019-12
  amount_m: 1.0
  lead_investors:
  - null
  source: https://www.crunchbase.com/organization/mindsdb
- stage: Seed
  date: 2020-04
  amount_m: 3.0
  lead_investors:
  - OpenOcean
  source: https://www.crunchbase.com/organization/mindsdb
- stage: Series A
  date: 2022
  amount_m: 16.5
  lead_investors:
  - Benchmark
  source: https://mindsdb.com/newsroom/mindsdb-raises-16-5m-from-benchmark-to-put-machine-learning-into-more-applications
- stage: Series B
  date: 2023
  amount_m: 25.0
  lead_investors:
  - Mayfield
  source: https://mindsdb.com/newsroom/mindsdb-raises-25-million-led-by-mayfield-to-supercharge-developers-with-its-cloud-for-serving-artificial-intelligence-logic
- stage: Strategic
  date: 2023-08
  lead_investors:
  - NVentures (NVIDIA)
  source: https://mindsdb.com/newsroom/mindsdb-secures-funding-from-nvidia-to-make-ai-more-accessible-to-all-businesses
confidence: medium
last_updated: 2026-03-23
linkedin: https://www.linkedin.com/company/mindsdb
linkedin_verified: false
crunchbase: https://www.crunchbase.com/organization/mindsdb
---

# MindsDB

## Overview

MindsDB is an open-source AI platform that enables developers to integrate machine learning into applications using familiar SQL syntax. Founded in 2017 by Jorge Torres and Adam Carrigan, inspired by Iain M. Banks's Culture series, the company connects AI models directly to data sources, making ML accessible without requiring specialized expertise. MindsDB is backed by $50M+ in funding from Benchmark, Mayfield, Y Combinator, and NVIDIA's NVentures. The platform supports integration with hundreds of data sources and AI models.

## Funding History

| Round     | Date     | Amount | Lead Investor(s)     |
|-----------|----------|--------|----------------------|
| Seed (x3) | 2017-20 | $4.3M  | OpenOcean            |
| Series A  | 2022     | $16.5M | Benchmark            |
| Series B  | 2023     | $25M   | Mayfield             |
| Strategic | Aug 2023 | --     | NVentures (NVIDIA)   |

**Total raised:** ~$54M
