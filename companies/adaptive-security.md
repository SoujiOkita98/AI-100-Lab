---
name: Adaptive Security
status: active
founded: 2024
website: https://www.adaptivesecurity.com
sector:
- AI cybersecurity
- deepfake defense
- social engineering prevention
- security awareness training
- enterprise security
one_liner: AI-powered cybersecurity platform defending enterprises against deepfakes, AI-generated vishing, smishing, and social engineering attacks.
logo: ''
total_raised_m: 146.5
latest_valuation_m:
funding_rounds:
- stage: Seed
  date: 2024-01
  amount_m: 12
  valuation_m:
  lead_investors:
  - Abstract Ventures
  source: https://www.crunchbase.com/organization/adaptive-security
  notes: Initial funding to develop AI social engineering defense platform. Exact date and details approximate.
- stage: Series A
  date: 2025-04
  amount_m: 43
  valuation_m:
  lead_investors:
  - Andreessen Horowitz (a16z)
  - OpenAI Startup Fund
  source: https://www.prnewswire.com/news-releases/adaptive-security-raises-43m-a16z-120000880.html
  notes: OpenAI's first and only cybersecurity investment. Additional participation from Abstract Ventures, Eniac Ventures, CrossBeam Ventures, K5, and executives from Google, Workday, Shopify, Plaid, Paxos.
- stage: Series A Extension
  date: 2025-09
  amount_m: 12
  valuation_m:
  lead_investors:
  - OpenAI Startup Fund
  source: https://www.adaptivesecurity.com/blog/openai-increases-investment
  notes: OpenAI doubled down, bringing total Series A to $55M.
- stage: Series B
  date: 2025-12
  amount_m: 81
  valuation_m:
  lead_investors:
  - Bain Capital Ventures
  source: https://www.prnewswire.com/news-releases/adaptive-security-raises-81-million-series-b-to-stop-ai-powered-cyber-threats-302643174.html
  notes: Participation from NVentures (NVIDIA), OpenAI Startup Fund, a16z, Abstract Ventures, Capital One Ventures, Citi Ventures. Third financing in one year.
founders:
- name: Brian Long
  role: Co-Founder & CEO
  background: Serial entrepreneur. Previously co-founded and grew Attentive to $500M in annual revenue. Recognized the threat of AI-enabled impersonation early and pivoted to build AI defenses.
  origin: American
- name: Andrew Jones
  role: Co-Founder
  background: Serial entrepreneur. Previously co-founded Attentive with Brian Long, scaling it to $500M ARR. Brings deep enterprise sales and product experience.
  origin: American (uncertain)
team_composition: The founding duo previously built Attentive into a major enterprise software company ($500M ARR), giving them rare experience in scaling enterprise security products. The investor backing from OpenAI and a16z signals strong technical validation.
business_model: Enterprise SaaS. Adaptive provides AI-powered security awareness and defense tools that simulate and detect deepfake attacks (voice, video, email). Enterprises subscribe to the platform for employee training, real-time threat detection, and social engineering defense. The company reports a world-class NPS score of 94.
products_and_models:
- 'AI deepfake simulation: Creates realistic AI-generated deepfake scenarios for employee training and awareness.'
- 'Vishing defense: Protects against AI-powered voice phishing attacks over phone calls.'
- 'Smishing defense: Detects and prevents AI-generated SMS phishing.'
- 'Real-time social engineering detection: AI-powered monitoring for enterprise communication channels.'
customers_and_partnerships:
- 500+ enterprise customers within first year of public launch (January 2025)
- First State Bank
- Podium
- Dallas Mavericks
- BMC
- OpenAI (investor and strategic partner)
- NVIDIA (investor via NVentures)
revenue_signals: 500+ enterprise customers within 12 months of launch. Three funding rounds in a single year totaling $136M+ suggests very strong growth metrics. The founders' track record of building Attentive to $500M ARR adds credibility to revenue scaling.
competitors:
- KnowBe4 (security awareness training)
- Proofpoint (email security)
- Abnormal Security (AI email security)
- Pindrop (voice fraud detection)
- Reality Defender (deepfake detection)
sources:
- https://www.prnewswire.com/news-releases/adaptive-security-raises-43m-a16z-120000880.html
- https://www.prnewswire.com/news-releases/adaptive-security-raises-81-million-series-b-to-stop-ai-powered-cyber-threats-302643174.html
- https://www.adaptivesecurity.com/blog/openai-increases-investment
- https://www.securityweek.com/serial-entrepreneurs-raise-43m-to-counter-ai-deepfakes-social-engineering/
- https://cybermagazine.com/articles/adaptive-security-inside-openais-first-cyber-investment
- https://www.biometricupdate.com/202512/new-series-b-round-brings-adaptives-total-capital-raised-to-146-5-million
last_updated: 2026-03-24
confidence: high
confidence_notes: Funding rounds well-sourced from company press releases, PR Newswire, SecurityWeek. Total raised is $146.5M per Biometric Update (slightly higher than initial $136M estimate, likely due to small earlier rounds). Founder backgrounds confirmed across multiple sources. Valuation figures not publicly disclosed for individual rounds.
website_verified: true
twitter: '@adaptaboringsec'
linkedin: https://www.linkedin.com/company/adaptive-security
crunchbase: https://www.crunchbase.com/organization/adaptive-security
crunchbase_verified: true
headquarters: New York, NY
linkedin_verified: true
---

# Adaptive Security

## Overview

Adaptive Security is a New York-based cybersecurity startup founded in 2024 by Brian Long and Andrew Jones, the duo who previously built Attentive to $500M in annual revenue. The company builds AI-powered tools to defend enterprises against the rising threat of AI-generated social engineering attacks, including deepfake voice calls (vishing), AI-generated SMS phishing (smishing), and deepfake video.

Adaptive raised $146.5M across four rounds in roughly one year, backed by OpenAI (their first cybersecurity investment), a16z, Bain Capital Ventures, and NVIDIA. The company grew to 500+ enterprise customers within 12 months of its January 2025 public launch.

## Funding story

**Early 2024 -- Seed (~$12M).** Abstract Ventures led initial funding to develop the AI social engineering defense platform.

**April 2025 -- Series A ($43M).** Co-led by a16z and OpenAI Startup Fund. This was OpenAI's first and only cybersecurity investment, signaling that the creators of the AI threat also see the urgency of AI defense.

**September 2025 -- Series A Extension ($12M).** OpenAI doubled down, bringing total Series A to $55M.

**December 2025 -- Series B ($81M).** Bain Capital Ventures led. NVentures (NVIDIA), OpenAI, a16z, Capital One Ventures, and Citi Ventures all participated. Three financing rounds in one year is exceptional velocity.

## What makes them interesting

1. **OpenAI's only cybersecurity bet.** The company that created the deepfake threat is investing in defending against it. This is both a strong signal and a unique strategic advantage.

2. **Proven founders.** Long and Jones built Attentive to $500M ARR. They know how to scale enterprise software and are applying that playbook to AI security.

3. **Market timing.** AI-powered social engineering is an exploding threat category. Legacy security training tools were not designed for AI-generated attacks, creating a greenfield opportunity.
