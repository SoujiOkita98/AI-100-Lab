---
name: Kyutai
website: https://kyutai.org
one_liner: French open-science AI research lab focused on voice and audio AI, backed by Xavier Niel, Rodolphe Saade, and Eric
  Schmidt
headquarters: Paris, France
founded: '2023'
sector: Artificial Intelligence
sub_sector: Foundation Models / Voice AI / Audio AI
business_model: Nonprofit research lab (open-source models)
status: Active
headcount_estimate: ~50
founders:
- name: Xavier Niel
  role: Co-founder & Primary Backer
  background: Founder of Iliad Group / Free (telecom); Founder of Station F
  origin: French
- name: Rodolphe Saade
  role: Co-founder & Backer
  background: CEO of CMA CGM Group (shipping & logistics)
  origin: French
- name: Eric Schmidt
  role: Co-founder & Backer
  background: Former CEO & Chairman of Google; Schmidt Futures
  origin: American
key_leadership:
- name: Patrick Perez
  role: Director
  prior_experience:
  - Valeo (VP of AI)
  source: https://www.actuia.com/en/actors/kyutai/
- name: Edouard Grave
  role: Research Scientist
  prior_experience:
  - Meta AI (FAIR)
- name: Herve Jegou
  role: Research Scientist
  prior_experience:
  - Meta AI (FAIR)
- name: Laurent Mazare
  role: Research Scientist
  prior_experience:
  - Google DeepMind
- name: Neil Zeghidour
  role: Research Scientist
  prior_experience:
  - Google DeepMind
- name: Alexandre Defossez
  role: Research Scientist
  prior_experience:
  - Meta AI (FAIR)
scientific_advisory_board:
- Yejin Choi
- Yann LeCun (Chief AI Scientist, Meta)
- Bernhard Scholkopf
funding_rounds:
- stage: Initial Endowment
  date: 2023-11
  amount_m: 0.0
  other_investors:
  - Xavier Niel / Iliad Group (~EUR100M)
  - Rodolphe Saade / CMA CGM Group (~EUR100M)
  - Eric Schmidt / Schmidt Futures
  - Other unnamed donors
  source: https://techcrunch.com/2023/11/17/kyutai-is-an-french-ai-research-lab-with-a-330-million-budget-that-will-make-everything-open-source/
total_raised_usd: ~330M
key_products:
- name: Moshi
  description: First speech-native dialogue system with ~200ms latency; processes speech directly without text conversion
  launched: 2024-07
  open_source: true
  source: https://github.com/kyutai-labs/moshi
- name: Mimi
  description: State-of-the-art streaming neural audio codec used by Moshi
  open_source: true
- name: Hibiki
  description: Streaming simultaneous voice translation model preserving speaker voice
  launched: 2025-02
  open_source: true
  source: https://github.com/kyutai-labs/hibiki
- name: Helium-1
  description: Lightweight multilingual model supporting six European languages, designed for mobile devices
  launched: 2025-01
  open_source: true
notes: 'Kyutai is structured as a privately funded nonprofit research lab, not a traditional VC-backed startup. Launched on
  November 17, 2023 at Station F in Paris. All research and models are released as open source. The core research team was
  assembled primarily from Meta AI (FAIR), Google DeepMind, and Inria.

  '
confidence: high
last_updated: 2026-03-23
linkedin: https://www.linkedin.com/company/kyutai/
linkedin_verified: true
---
