---
name: Runware
founded: 2023
headquarters: San Francisco, CA
website: https://runware.ai
sector: AI Inference Infrastructure
one_liner: Generative AI inference API — one API for all AI image/video/audio generation.
status: active
funding_rounds:
- stage: Seed
  date: 2024-10
  amount_m: 13
  lead_investors:
  - Insight Partners
  - a16z Speedrun
  - Begin Capital
  - Zero Prime
  source: https://runware.ai/blog/runware-raises-13m-seed-to-scale-ai-generation-platform-for-developers
- stage: Series A
  date: 2025-12
  amount_m: 50
  lead_investors:
  - Dawn Capital
  - Comcast Ventures
  - Speedinvest
  - Insight Partners
  source: https://techcrunch.com/2025/12/11/runware-raises-50m-series-a/
last_updated: 2026-03-21
confidence: medium
data_notes: 'Needs verification: founders, source_urls, headquarters. Profile may be incomplete.'
website_verified: true
crunchbase: https://www.crunchbase.com/organization/runware
crunchbase_verified: true
total_raised_m: 66.0
founders:
- name: Flaviu Radulescu
  role: Co-Founder & CEO
  background: 20+ years experience in big data and cloud platforms
  origin: Romanian
- name: Ioana Hreninciuc
  role: Co-Founder & COO
  background: Operations and GTM focus
  origin: Romanian
linkedin: https://www.linkedin.com/company/runware/
linkedin_verified: true
---

# Runware

Generative AI inference API — one API for all AI image/video/audio generation.
