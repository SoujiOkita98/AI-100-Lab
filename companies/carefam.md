---
name: Carefam
status: active
founded: 2024
website: https://www.carefam.com
headquarters: Tel Aviv, Israel
sector:
- AI healthcare
- HR automation
- agentic AI
one_liner: Israeli AI startup automating healthcare hiring with conversational AI agents that handle candidate outreach, scheduling,
  credential screening, and onboarding for hospitals and care facilities.
logo: null
total_raised_m: 14.5
latest_valuation_m: null
funding_rounds:
- stage: Seed
  date: 2026-03
  amount_m: 14.5
  valuation_m: null
  lead_investors:
  - Pitango HealthTech
  source: https://www.calcalistech.com/ctechnews/article/h1hd004eq11g
  notes: Emerged from stealth in March 2026. Earlier round backed by Emerge. Platform shows 900% growth in one year with hundreds
    of US healthcare providers already onboard.
founders:
- name: Matan Hoffmann
  role: CEO & Co-founder
  background: Former IDF Special Forces. Healthcare technology entrepreneur.
  origin: Israeli.
- name: Eyal Shulman
  role: CTO & Co-founder
  background: Former Unit 8200 (IDF elite intelligence and cybersecurity division). Engineering and cybersecurity background.
  origin: Israeli.
confidence: high
last_updated: 2026-03-23
linkedin: https://www.linkedin.com/company/carefam/
---

# Carefam

## Overview
Carefam is an Israeli AI startup that emerged from stealth in March 2026 with $14.5M in funding led by Pitango HealthTech. The company builds an AI-powered platform for long-term care and healthcare human resources, deploying intelligent agents that automate candidate outreach, scheduling, credential screening, and onboarding for hospitals and care facilities. Co-founded by Matan Hoffmann (CEO, ex-IDF Special Forces) and Eyal Shulman (CTO, ex-Unit 8200), Carefam has achieved 900% growth in a single year with hundreds of US healthcare providers already using the platform. The company addresses the critical staffing shortage in US healthcare by freeing up HR teams from repetitive hiring workflows.
