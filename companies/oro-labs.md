---
name: ORO Labs
status: active
founded: 2020
website: https://www.orolabs.ai
headquarters: Palo Alto, California, United States
sector:
- enterprise AI
- procurement automation
- agentic AI
one_liner: AI-native procurement orchestration platform automating procurement requests, supplier onboarding, and compliance checks for global enterprises like Coca-Cola, Siemens Energy, and Novartis.
logo: null
total_raised_m: 160
latest_valuation_m: null
funding_rounds:
- stage: Series C
  date: 2026-03
  amount_m: 100
  valuation_m: null
  lead_investors:
  - Brighton Park Capital
  - Goldman Sachs Alternatives
  source: https://fortune.com/2026/03/12/oro-labs-corporate-procurement-ai-efficiency/
  notes: Continued participation from Norwest Venture Partners, B Capital, XYZ Capital, and Felicis. 300% revenue growth reported in 2024-2025.
- stage: Series B
  date: 2023-11
  amount_m: 34
  valuation_m: null
  lead_investors:
  - Norwest Venture Partners
  source: https://techcrunch.com/2023/11/01/procurement-management-software-startup-oro-labs-lands-34m-investment/
  notes: null
founders:
- name: Sudhir Bhojwani
  role: CEO & Co-founder
  background: Enterprise software executive with deep expertise in procurement technology.
  origin: Indian.
- name: Lalitha Rajagopalan
  role: Co-founder
  background: Enterprise technology executive focused on procurement automation.
  origin: Indian.
- name: Yuan Tung
  role: Co-founder
  background: Enterprise technology executive.
  origin: Chinese.
confidence: high
last_updated: 2026-03-23
---

# ORO Labs

## Overview
ORO Labs is a Palo Alto-based startup founded in 2020 that provides an AI-native procurement orchestration platform. The platform coordinates people, processes, and systems to automate procurement front-end processes including new procurement requests, supplier onboarding, and compliance checks. ORO Labs' software is used by enterprises including Coca-Cola, Siemens Energy, and Novartis, with deployments across organizations operating in more than 100 countries. The company has raised $160M total and reported 300% revenue growth, demonstrating strong product-market fit in the enterprise procurement automation space.
