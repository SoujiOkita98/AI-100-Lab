---
name: Onlook
founded: 2024
headquarters: San Francisco, CA
website: https://onlook.dev
sector: AI Developer Tools / Design-to-Code
one_liner: Open-source visual editor for React apps that lets designers and developers collaborate in real time
status: Active
total_raised_m: 0.5
founders:
  - name: "Daniel Farrell"
    role: "Co-Founder (Design & Growth)"
    background: "Designer for over a decade; first 100 employee at Bird; former Head of Growth; ex-DIMO, Defy Marketing"
    origin: "United States"
  - name: "Kiet Ho"
    role: "Co-Founder (Engineering)"
    background: "Ex-Amazon; maintained design system at ServiceNow"
    origin: "Vietnam"
funding_rounds:
- stage: Seed
  date: 2025-03
  amount_m: 0.5
  lead_investors:
  - Y Combinator
notable_investors:
- Y Combinator
tags:
- ai-developer-tools
- design-to-code
- open-source
- react
- visual-editor
- yc-w25
yc_batch: W25
linkedin: https://www.linkedin.com/company/onlook-dev
confidence: Low
last_updated: 2026-03-23
---

# Onlook

## Overview

Onlook is an open-source visual editor for React applications from YC's Winter 2025 batch. The tool enables designers and developers to visually edit React apps and collaborate in real time, bridging the gap between design and code.

## Funding History

| Round | Date       | Amount | Lead Investor |
|-------|------------|--------|---------------|
| Seed  | March 2025 | $500K  | Y Combinator  |

## Sources

- [Y Combinator: Onlook Company Page](https://www.ycombinator.com/companies/onlook)
