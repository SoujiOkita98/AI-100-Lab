---
name: Nomagic
founded: 2017
headquarters: Warsaw, Poland
website: https://nomagic.ai
sector:
- physical-ai
- robotics
- warehouse-automation
one_liner: Polish AI robotics company building intelligent pick-and-place robotic arms for warehouse automation.
status: active
total_raised_m: 84
founders:
- name: Kacper Nowicki
  role: CEO
  background: Former Google engineer (5+ years); built startup in California before returning to Poland
  origin: Polish
- name: Marek Cygan
  role: Co-founder
  background: Academic and robotics researcher
  origin: Polish
- name: Tristan D'Orgeval
  role: Co-founder
  background: Entrepreneurial background
  origin: French
funding_rounds:
- stage: Series B
  date: 2025-02
  amount_m: 44
  lead_investors:
  - EBRD Venture Capital
  source: https://techcrunch.com/2025/02/26/nomagic-picks-up-44m-for-its-ai-powered-robotic-arms/
- stage: Series B Extension
  date: 2026-01
  amount_m: 10
  lead_investors:
  - Cogito Capital Partners
  source: https://www.eu-startups.com/2026/01/polish-robotics-startup-nomagic-secures-e8-3-million-to-scale-physical-ai-operations-in-the-us/
confidence: high
last_updated: 2026-03-23
linkedin: https://www.linkedin.com/company/nomagic/
linkedin_verified: true
---

# Nomagic

## Overview
Nomagic is a Polish AI robotics company that builds intelligent robotic arms for picking, packing, and sorting in logistics warehouses. The company's robots learn from millions of real operational tasks in 24/7 environments, building an adaptable Physical AI platform. Founded in 2017 by former Google engineer Kacper Nowicki, Nomagic has achieved 220% ARR growth and serves major logistics customers across Europe. Backed by Khosla Ventures, EBRD, Almaz Capital, and the European Investment Bank, the company is expanding into the US market. Nomagic is a rare example of a Central European AI robotics success story.
