---
name: Catena Labs
status: active
founded: 2021
website: https://catenalabs.com
sector:
- AI finance
- stablecoins
- agent payments
- financial infrastructure
- crypto/blockchain
one_liner: Building the first AI-native financial institution designed to serve autonomous AI agents, enabling machine-to-machine
  payments and financial services.
logo: ''
total_raised_m: 18
latest_valuation_m: null
funding_rounds:
- stage: Seed
  date: 2025-05
  amount_m: 18
  valuation_m: null
  lead_investors:
  - a16z crypto
  source: https://www.businesswire.com/news/home/20250520361792/en/Circle-Co-Founder-Sean-Neville-Takes-Catena-Labs-Out-of-Stealth-with-Plans-to-Build-the-First-AI-Native-Financial-Institution
  notes: Emerged from stealth May 2025. Participation from Breyer Capital, Circle Ventures, Coinbase Ventures, CoinFund, Pillar
    VC, Stanford Engineering VF. Angels include Tom Brady, Bradley Horowitz, Hamel Husain, Kevin Lin, Sam Palmisano, Balaji
    Srinivasan.
founders:
- name: Sean Neville
  role: Co-Founder & CEO
  background: Co-Founder of Circle, where he helped build USDC into the world's second-largest stablecoin. Deep expertise
    in regulated financial infrastructure, digital payments, and stablecoin economics. One of the architects of the stablecoin
    ecosystem.
  origin: American
- name: Matt Venables
  role: Co-Founder
  background: Circle alumni who helped build USDC. Brings experience in financial product development and regulatory compliance
    in digital payments.
  origin: American
team_composition: Both founders are Circle veterans who built USDC, giving them unmatched credibility in regulated digital
  financial infrastructure. Their experience navigating financial regulation is critical for building a licensed AI-native
  financial institution.
business_model: Regulated financial institution for AI agents. Catena is pursuing financial institution licensing to offer
  banking, payments, and financial services specifically designed for autonomous AI agents. The business model involves transaction
  fees on agent-to-agent and agent-to-human payments, potentially using stablecoins as the settlement layer. The company has
  open-sourced the Agent Commerce Kit (ACK) for AI agent identity and payments to build ecosystem adoption.
products_and_models:
- 'Agent Commerce Kit (ACK): Open-source protocols for AI agent identity and payments.'
- 'AI-native financial institution: Pursuing regulatory licensing to offer banking and payment services for AI agents.'
- 'Agent identity verification: Systems for verifying and authenticating AI agents in financial transactions.'
- 'Machine-to-machine payments: Infrastructure for autonomous AI agents to transact with each other and with humans.'
customers_and_partnerships:
- Circle Ventures (strategic investor, USDC ecosystem)
- Coinbase Ventures (strategic investor, crypto infrastructure)
- a16z crypto (lead investor, Web3 ecosystem)
revenue_signals: Pre-revenue. Company emerged from stealth in May 2025 and is pursuing financial institution licensing. The
  revenue opportunity depends on the growth of the AI agent economy and the need for agent-to-agent financial services. No
  disclosed customers or revenue yet.
competitors:
- Stripe (payments infrastructure, expanding to AI)
- PayPal (digital payments)
- Skyfire (AI agent payments)
- Payman AI (AI agent payments)
- Circle (USDC stablecoin infrastructure -- founders' former company)
sources:
- https://www.businesswire.com/news/home/20250520361792/en/Circle-Co-Founder-Sean-Neville-Takes-Catena-Labs-Out-of-Stealth-with-Plans-to-Build-the-First-AI-Native-Financial-Institution
- https://www.pymnts.com/news/investment-tracker/2025/catena-labs-raises-18-million-to-build-ai-native-financial-institution-for-agents/
- https://cointelegraph.com/news/catena-labs-raises-18m-ai-native-financial-institution
- https://catenalabs.com/blog/announcing-catena-labs/
- https://blockeden.xyz/blog/2025/10/28/catena-labs-building-the-first-ai-native-financial-institution/
last_updated: 2026-03-24
confidence: medium
confidence_notes: Seed round well-sourced from BusinessWire press release, PYMNTS, CoinTelegraph. Founder backgrounds confirmed
  as Circle co-founders. Company is early stage with no disclosed revenue or customers. Financial institution licensing status
  unclear. The thesis depends on AI agent economy growth, which is still speculative.
website_verified: true
twitter: '@catenalabs'
linkedin: https://www.linkedin.com/company/catena-labs
crunchbase: https://www.crunchbase.com/organization/catena-labs
crunchbase_verified: true
headquarters: Boston, MA
linkedin_verified: true
---

# Catena Labs

## Overview

Catena Labs is a Boston-based fintech company founded in 2021 by Sean Neville and Matt Venables, both co-founders of Circle (the company behind USDC, the world's second-largest stablecoin). Catena is building the first fully regulated AI-native financial institution designed to serve autonomous AI agents -- enabling machines to open accounts, make payments, and conduct financial transactions.

The company emerged from stealth in May 2025 with $18M in seed funding led by a16z crypto, with strategic participation from Circle Ventures, Coinbase Ventures, and notable angels including Tom Brady and Balaji Srinivasan.

## Funding story

**May 2025 -- Seed ($18M).** Led by a16z crypto. Participation from Breyer Capital, Circle Ventures, Coinbase Ventures, CoinFund, Pillar VC, and Stanford Engineering VF. Angel investors include Tom Brady, Bradley Horowitz, Kevin Lin (Twitch co-founder), Sam Palmisano (former IBM CEO), and Balaji Srinivasan. The company had been operating in stealth since 2021.

## What makes them interesting

1. **Circle founders building for AI agents.** Neville and Venables helped create USDC ($30B+ market cap). They understand regulated financial infrastructure at a level few founders can match, and they're applying that expertise to the emerging AI agent economy.

2. **First-mover on agent finance.** As AI agents become more autonomous, they will need financial identities, payment capabilities, and regulatory compliance. Catena is positioning to be the financial infrastructure layer for this new economy.

3. **Open-source ecosystem play.** The Agent Commerce Kit (ACK) builds developer adoption and standards, similar to how Circle's open USDC standard built ecosystem lock-in.

4. **The risk.** The AI agent economy is still largely theoretical. Whether autonomous agents will truly need regulated banking services (rather than just API keys to Stripe) remains to be proven. The timeline for mainstream agent commerce is uncertain.
